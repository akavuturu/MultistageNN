...
class RegressionNN:
    def __init__(self, t, x, layers, kappa, lt, ut, act):
        # TO IMPLEMENT: moving computation to gpu if possible
        ...
        # scale factor used in loss scaling and x scaling
        self.scale = tensor.max(torch.abs(x)) / 2
        scaled_x = x / self.scale

        # defining basic params
        self.t = t
        self.x = scaled_x
        self.lt = lt
        self.ut = ut
        ...
        # defining training params (weights, biases) as one list to optimize later
        self.weights, self.biases = self.init_params(layers)
        self.train_params = self.weights + self.biases
        
        # defining loss params 
        self.loss_scale = self.scale ** 2
        self.loss = []
        self.loss_init = self.loss_nn()

        # using Adam as the sole optimizer
    ...
    def init_params(self, layers):
        ...
        # iterate for each layer of nn
        for l in range(n - 1):
            # init weights with Xavier initialization given in_dim and out_dim
            W = self.w_init(layers[l], layers[l + 1])
            # init biases as all zero
            B = torch.zeros([1, layers[l + 1]], dtype=self.dtype)
            weights.append(W)
            biases.append(B)
        return weights, biases
    
    # returns array of random vals with Xavier initialization
    def w_init(self, in_dim, out_dim):
        xavier = np.sqrt(2 / (in_dim + out_dim))
        return nn.random.trunc_normal_(torch.zeros(in_dim, out_dim), 
                                std=xavier, a=-2*xavier, b=2*xavier, 
                                dtype=self.dtype)
    ...
    def build_net(self, X):
        ...
        W = weights[0]
        B = biases[0]
        H = self.act(torch.add(self.kappa * torch.mm(H, W), B))
    ...
    def loss(self):
        self.x_pred = self.build_net(self.t)
        loss_ = torch.mean(torch.square(self.x - self.x_pred))
        return loss_
    
    def adam_f(self):
        def f():
            loss_norm = self.loss()
            loss_val = loss_norm * self.loss_scale
            f.loss.append(loss_val)
            f.iter += 1

            if f.iter % 10 == 0:
                print("Iter:", f.iter, ", loss: ", loss_val)
            return loss_norm
        f.iter = 1
        f.loss = []
        return f
    ...
    def train(self, iter):
        ...
        adam_f = self.adam_f()
        # for i in range(iter):
        #     self.optimizer(params, lr=0.001)
        return adam_f

    def predict(self, t):
        x_pred = self.neural_net(t) * self.scale
        return x_pred